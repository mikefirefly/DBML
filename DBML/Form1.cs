...
namespace DBML
{
    public partial class MainForm : Form
    {
        ...
        string defaultCfgFile = Application.StartupPath + @"\DBML-baseline.conf";
        ...
        private void MainForm_Load(object sender, EventArgs e)
        {
            ...
            {
                startupError = true;
                MessageBox.Show("DBML-baseline.conf file not found in the application folder. This file must exist to provide the baseline DOSBox configuration. Application will terminate.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                Application.Exit();
            }
            ...
            if (File.Exists(entriesFile))
            {
                ...
                if (Entries.Count > 0) lbEntryList.SelectedIndex = 0;
            }
        }
        ...
        private void lbEntryList_DragDrop(object sender, DragEventArgs e)
        {
            ...
            if (lbEntryList.Items.Cast<Entry>().Any(x => x.FriendlyName == en.FriendlyName))
                MessageBox.Show("It looks like this entry already exists on the list!", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void StartTheGame(int bits)
        {
            ...
            if (notFoundLines.Count > 0)
                if (MessageBox.Show("Failed to find the following entries in the DOSBox configuration file: \n\n" +
                    string.Join(Environment.NewLine, notFoundLines) + "\n\nI don't know which section of the configuration file " +
                    "they belong to so I can't insert them automatically. Please see the baseline configuration file and " +
                    "add these entries in appropriate section manually.\n\nDo you want to run " + lbEntryList.Text + " now without these entries?", "Warning", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.No)
                    return;
            ...
            p.StartInfo.Arguments = $"-conf \"{tmpCfgFile}\" \"{tbPath.Text}\\{tmpBatFile}\"";
